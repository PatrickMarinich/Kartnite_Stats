...
def getKVRLeaderBoards(dfKVR):
  #print(dfKVR)
  #current is row 0,
  # all time is row 1

  #leaderboard object
  dfKVRCurrent = pd.DataFrame({'Player': [], 'Current KVR':[]})
  dfKVRAllTime = pd.DataFrame({'Player': [], 'All Time High KVR':[]})

  #for each player 
  counter = 0
  for player in set(dfKVR.columns.values.tolist()):
    #print(player)
    #adds KVRs to the new leaderboard, all time and current
    if player != 'KVR Stats':
      dfKVRCurrent.loc[counter] = [player, int(dfKVR.at[0, player])]
      dfKVRAllTime.loc[counter] = [player, int(dfKVR.at[1, player])]
      counter = counter + 1
  #sorts the leaderboards
  dfKVRCurrent = dfKVRCurrent.sort_values(['Current KVR', 'Player'],  ascending=[0, 1])
  dfKVRAllTime = dfKVRAllTime.sort_values(['All Time High KVR', 'Player'],  ascending=[0, 1])

  #print(dfKVRCurrent)
  #print(dfKVRAllTime)
  return dfKVRCurrent,dfKVRAllTime
